...
const sideMenu = computed(() => {
    sideMenuStore.menu.forEach((item, key) => {
        if (item.link === route.url) {
            item.active = true;
        } else {
            item.active = false;
        }
        if (item.subMenu && item.subMenu.length > 0) {
            item.subMenu.forEach((subItem, subKey) => {
                if (subItem.link === route.url) {
                    subItem.active = true;
                    item.activeDropdownOpen = true;
                } else {
                ...
                <div class="flex-1 px-3 space-y-1 bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                    <ul class="pb-2 space-y-2">
                        ...
                        <template v-for="(menu, menuKey) in formattedMenu">
                            <li v-if="menu.subMenu && menu.subMenu.length > 0" :key="menuKey">
                                ...
                                <ul v-if="menu.activeDropdownOpen" class="py-2 space-y-2">
                        ...
                        <li>
                            ...
                            <svg class="w-6 h-6 text-gray-500 transition duration-75 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M2 10a8 8 0 018-8v8h8a8 8 0 11-16 0z"></path><path d="M12 2.252A8.014 8.014 0 0117.748 8H12V2.252z"></path></svg>
                            <span class="ml-3" sidebar-toggle-item>Dashboard111111</span>
                            </a>
                        </li>
                        <li>
                            <button type="button" class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700" aria-controls="dropdown-layouts" data-collapse-toggle="dropdown-layouts">
...
<!--                            <ul id="dropdown-crud" class="space-y-2 py-2 {{ if not (eq .Params.group "crud") }}hidden {{ end }}">-->
                            ...
                            <li>
<!--                                <a href="/" class="text-base text-gray-900 rounded-lg flex items-center p-2 group hover:bg-gray-100 transition duration-75 pl-11 dark:text-gray-200 dark:hover:bg-gray-700 {{ if eq $page_slug "products" }} bg-gray-100 dark:bg-gray-700 {{ end }}">Products</a>-->
                            ...
                            <li>
<!--                                <a href="/" class="text-base text-gray-900 rounded-lg flex items-center p-2 group hover:bg-gray-100 transition duration-75 pl-11 dark:text-gray-200 dark:hover:bg-gray-700 {{ if eq $page_slug "users" }} bg-gray-100 dark:bg-gray-700 {{ end }}">Users</a>-->
...
<!--                        <ul id="dropdown-playground" class="space-y-2 py-2 {{ if not (eq .Params.group "playground") }}hidden {{ end }}">-->
                        <ul id="dropdown-playground" class="space-y-2 py-2">
                        <li>
<!--                            <a href="/" class="text-base text-gray-900 rounded-lg flex items-center p-2 group hover:bg-gray-100 transition duration-75 pl-11 dark:text-gray-200 dark:hover:bg-gray-700 {{ if eq $page_slug "stacked" }} bg-gray-100 dark:bg-gray-700 {{ end }}">Stacked</a>-->
                        ...
                        <li>
<!--                            <a href="/" class="text-base text-gray-900 rounded-lg flex items-center p-2 group hover:bg-gray-100 transition duration-75 pl-11 dark:text-gray-200 dark:hover:bg-gray-700 {{ if eq $page_slug "sidebar" }} bg-gray-100 dark:bg-gray-700 {{ end }}">Sidebar</a>-->
